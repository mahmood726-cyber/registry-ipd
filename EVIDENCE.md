...
| **Independent validation vs published HR** | 16 high-confidence published HRs (PubMed abstracts): reconstructed-vs-published median fold 1.165, reconstructed inside the published 95% CI 13/16; the two INDEPENDENT held-out sources (registry vs published) themselves agree only 13/16 | `python harvest/pubmed_validation.py` |
| **Full-cohort independent validation** | 20 high-confidence published HRs across the reconstructed cohort (12 with **no** registry HR — pure registry-independent): recon-vs-published median fold 1.097, recon inside the published 95% CI 17/20 | `node validate/cohort_recon_export.js && python harvest/cohort_pubmed.py` |
| **Uncertainty on real registry curves** | the reconstructed 95% credible interval covers the INDEPENDENT published HR 17/20 (85%) on real coarse curves; CI width comparable to the gold standard (ratio 2.556) | `node validate/cohort_uncertainty_validation.js` |
| **Independent median validation** | reconstructed per-arm median vs the PUBLISHED median (PubMed, endpoint-matched): 10 arm-medians across 5 trials, median fold 1.071 (within 20%: 7/10) — independent of the registry and the HR | `python harvest/pubmed_medians.py && node validate/pubmed_median_validation.js` |
| **Registry-posted median cross-check** | reconstructed per-arm median vs the AACT-posted same-endpoint median: 22 arm-medians across 11 curve-consistent trials, median fold 1.133 (within 20%: 13/22); surfaced 2 trials where the registry's posted curve and median disagree | `python harvest/registry_medians.py && node validate/registry_median_validation.js` |
| **Expanded held-out HR validation** | 57 validation-grade trials scored vs registry HR (was 30); reconstructed HR inside the registry's posted 95% CI **47/57** (curve-sourced 24/25). Endpoint-aware sibling-HR recovery; OS-vs-PFS mismatches dropped | `python harvest/backfill_validation_hr.py && node validate/gallery_expanded.js` |
